...
class Neo4jStoreAdapter(GraphStoreAdapter):
    ...
    def get_document_vertex(self, doc_name: str) -> Vertex:
        ...
        query = f"""
        MATCH (n:{GraphElemType.DOCUMENT.value})
        WHERE n.name = $doc_name
        RETURN n LIMIT 1
        """
    ...
    def create_graph(self, graph_name: str):
        ...
        indexes = [
            (
                f"CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.ENTITY.value}) "
                "ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS "
                f"FOR (n:{GraphElemType.ENTITY.value}) ON (n._community_id)"
            ),
            (
                f"CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.CHUNK.value}) "
                "ON (n.id)"
            ),
            (
                f"CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.CHUNK.value}) "
            ...
            (
                f"CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.DOCUMENT.value}) "
                "ON (n.id)"
            ),
            (
                f"CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.DOCUMENT.value}) "
