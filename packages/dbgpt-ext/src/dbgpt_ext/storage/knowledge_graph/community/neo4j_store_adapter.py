    ...
    def create_graph(self, graph_name: str):
        ...
        indexes = [
            (
                ...
                f"{GraphElemType.ENTITY.value}) ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:"
                f"{GraphElemType.ENTITY.value}) ON (n._community_id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:"
                f"{GraphElemType.CHUNK.value}) ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:"
                f"{GraphElemType.CHUNK.value}) ON (n.content)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:"
                f"{GraphElemType.DOCUMENT.value}) ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:"
                f"{GraphElemType.DOCUMENT.value}) ON (n.name)"
            ),
        ]
