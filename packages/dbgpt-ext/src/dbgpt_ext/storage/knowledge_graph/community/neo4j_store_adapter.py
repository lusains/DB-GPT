...
class Neo4jStoreAdapter(GraphStoreAdapter):
    ...
    def get_document_vertex(self, doc_name: str) -> Vertex:
        ...
        query = (
            f"MATCH (n:{GraphElemType.DOCUMENT.value}) WHERE n.name = $doc_name "
            "RETURN n LIMIT 1"
        )
    ...
    def delete_document(self, chunk_id: str) -> None:
        ...
        MATCH (m:{GraphElemType.ENTITY.value})
        -[r:{GraphElemType.RELATION.value}]
    ...
    def create_graph(self, graph_name: str):
        ...
        indexes = [
            (
                ...
                f"(n:{GraphElemType.ENTITY.value}) ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR "
                f"(n:{GraphElemType.ENTITY.value}) ON (n._community_id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR "
                f"(n:{GraphElemType.CHUNK.value}) ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR "
                f"(n:{GraphElemType.CHUNK.value}) ON (n.content)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR "
                f"(n:{GraphElemType.DOCUMENT.value}) ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR "
                f"(n:{GraphElemType.DOCUMENT.value}) ON (n.name)"
            ),
        ]
