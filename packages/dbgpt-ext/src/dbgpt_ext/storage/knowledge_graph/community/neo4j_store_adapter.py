...
class Neo4jStoreAdapter(GraphStoreAdapter):
    ...
    def get_document_vertex(self, doc_name: str) -> Vertex:
        ...
        query = (
            f"MATCH (n:{GraphElemType.DOCUMENT.value}) "
    ...
    def delete_document(self, chunk_id: str) -> None:
        ...
        del_relation_query = (
            f"MATCH (m:{GraphElemType.ENTITY.value})-[r:{GraphElemType.RELATION.value}]"
            f"-(n:{GraphElemType.ENTITY.value}) "
    ...
    def create_graph(self, graph_name: str):
        ...
        indexes = [
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.ENTITY.value}) "
                "ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.ENTITY.value}) "
            ...
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.CHUNK.value}) "
                "ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.CHUNK.value}) "
            ...
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.DOCUMENT.value}) "
                "ON (n.id)"
            ),
            (
                "CREATE INDEX IF NOT EXISTS FOR (n:{GraphElemType.DOCUMENT.value}) "
